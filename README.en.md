<div align="center">

# Hi, I'm 禾几海 👋

**Java Backend Engineer · A programmer who works full-time for AI**

English | [中文](README.md)

</div>

---

<table>
  <tr>
    <td width="72%" valign="top">

### 👋 About me

- 🔍 Backend architecture & production stability
- 🧩 Turning AI-assisted coding into reusable engineering workflows with MCP tools & Skills
- 📊 Building tools to measure AI coding productivity — turning "feels faster" into data
- ✍️ Occasionally writing on my [blog](https://blog.celess.cn)
- 😶 daze & 🎧 music & 💻 code

    </td>
    <td width="28%" align="center" valign="middle">

<img src="./img/1628402933578.gif" width="200">

    </td>
  </tr>
</table>

### 🛠 Tech Stack

**Backend**: Java / Spring Boot / Spring Cloud / MySQL / Redis / RabbitMQ / Nacos / XXL-JOB
**Tuning**: JVM tuning · Arthas / MAT diagnostics · slow SQL optimization · high-concurrency performance
**Xinchuang**: DM (Dameng) / Kylin / TongWeb domestic-stack adaptation
**AI Engineering**: MCP tooling · Skills design · Prompt Engineering · AI coding productivity metrics
**Others**: Rust / TypeScript / Vue · Docker · CI/CD

### 🔥 Featured Project

| Project | Description |
|------|------|
| 📅 [SmartRiverCalendar](https://github.com/xiaohai2271/SmartRiverCalendar) | A cross-platform smart desktop calendar (Tauri 2 + Vue 3 + TS + Rust) with EWS/CalDAV sync, lunar holidays and smart reminders — delivered through a structured AI-assisted workflow (MCP + Skills + multi-agent collaboration) |

### 📫 Find me

[![Blog](https://img.shields.io/badge/Blog-blog.celess.cn-1a73e8?style=flat-square)](https://blog.celess.cn)
[![GitHub](https://img.shields.io/badge/GitHub-xiaohai2271-24292e?style=flat-square)](https://github.com/xiaohai2271)

---

<div align="center">

![Profile views](https://komarev.com/ghpvc/?username=xiaohai2271&style=flat-square&color=1a73e8)

</div>
